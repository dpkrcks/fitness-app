...
>
> **Done so far (code-complete, NOT runtime-tested, NOTHING committed — tip `4801985`):**
> - **Core auth (doc `14`)** — argon2id + JWT access + rotating DB refresh tokens; `register/login/refresh/logout/me`. (changelog 2026-06-30)
> - **Email OTP (doc `15` §B)** — `notifications` module (SMTP via nodemailer / dev-log fallback / SMS stub) + `otp.service`; `register` issues no tokens → `verify-otp` → tokens; unverified `login`→`403 EMAIL_NOT_VERIFIED`; `resend-otp`; throttler on `/auth/*`. Migration `…172018_auth_otp`. (changelog 2026-06-30)
> - **Profile onboarding (doc `15` §C)** — `me` module `GET/POST /me/profile` (name+DOB required, min-age 13). Migration `…174317_profiles`. (changelog 2026-06-30)
> - **Mobile OTP/onboarding flow** — `verify.tsx` + `profile-setup.tsx`, signup-no-token fix, login→verify routing, has-profile gate. (changelog 2026-06-30)
> - **Build state:** API `tsc` + `nest build` **clean**. Mobile: OTP/profile code compiles; **2 pre-existing errors remain from the redesign detour** (`_layout.tsx` missing `@react-navigation/native`; `text-field.tsx` TextInput overload).
>
> **▶️ RESUME HERE — do these in order:**
> 1. **Commit the auth work** (nothing is committed yet). Suggested split mirrors doc `15`'s proposed commits: shared-types schemas → api OTP → api profile/me → mobile flow.
> 2. **Runtime-test.** API: boot, `register` → read code from API console (no SMTP set) → `verify-otp` → tokens; `login` unverified → 403; resend cooldown (60s) + throttler caps. Mobile: `expo start` (also regenerates expo-router **typed routes** → clears any route-typing errors), then on Expo Go: register → verify → onboarding(name/DOB) → app; reopen persists; unverified login → verify.
> 3. **Fix the 2 pre-existing mobile redesign tsc errors** so mobile `tsc` is green (add `@react-navigation/native` dep; correct the TextInput focus-handler typing). Belongs to the redesign detour, not OTP.
> 4. **Google OAuth (doc `15` §A)** — deps `google-auth-library` (api) + `expo-auth-session`/`expo-crypto` (mobile, `expo-web-browser` already installed); `oauth_accounts` table + migration; `POST /auth/google` (verify ID token → find/link/create → our tokens); "Continue with Google" on login/register.
>
> **Still owed from the redesign detour (paused for OTP):** finish the mobile redesign polish + **write design-system doc `16`** (shared color tokens for future `apps/web`, splash→login, animations, health screen removed).
>
> **Mail delivery:** default = codes logged to API console (no setup). To send real email set `SMTP_*` (Mailpit local / Brevo free / Gmail app-password / prod relay) — no code change. Options documented to the user 2026-06-30.
> **Decisions resolved earlier:** DB = Neon (pooled runtime via adapter-pg, migrations via `DIRECT_URL`). Storage = **Cloudflare R2**. TS pinned 5.9.3 needs `ignoreDeprecations: "5.0"`.
> **Blockers:** none.
...
- ✅ Auth — Google OAuth: **ID-token pattern**, mobile via **`expo-auth-session`** (PKCE, stays in **Expo Go** — no EAS dev build); api verifies the ID token with `google-auth-library` and mints our own tokens. Native `@react-native-google-signin` deferred to a dev build. (`15`)
- ✅ Auth — OTP: **email only now**, system built **channel-agnostic** (delivery behind a provider interface; `nodemailer`→**Mailpit** in dev). **SMS deferred** — no free open-source SMS transport exists at scale. Apple Sign In deferred to iOS-release time (App Store 4.8). (`15`)
...
- [ ] **Auth: Google OAuth + email OTP** — 📋 detailed plan ready in `15-auth-oauth-otp-plan.md` (layers on top of `14`; **open-source/free** path; locked). Implement **after** doc 14's core.
  - [ ] DB: `passwordHash` nullable + `emailVerifiedAt`/`phone`/`phoneVerifiedAt`; new `oauth_accounts` + `verification_codes` models; migration `auth_oauth_otp`
  - [ ] OTP (email, channel-agnostic): `otp.service` (generate/hash/verify, 10m/5-attempt/60s-cooldown) + `notifications` delivery interface (`nodemailer` → **Mailpit** dev; SMS provider stubbed)
  - [ ] `POST /auth/register` issues NO tokens until verified; `POST /auth/verify-otp` → tokens; `POST /auth/resend-otp` (throttled); unverified login → `403 EMAIL_NOT_VERIFIED`
  - [ ] `POST /auth/google` → verify ID token (`google-auth-library`) → find/link/create user → our tokens
  - [ ] Post-OTP **onboarding + `Profile`** (1:1): `displayName`+`dateOfBirth` required (min age 13), units+body metrics skippable; `GET/POST /me/profile`; mobile onboarding screen + app "has-profile?" gate
  - [ ] `@nestjs/throttler` on `/auth/*`; shared-types `googleAuthSchema`/`verifyOtpSchema`/`resendOtpSchema`/`profileSchema` + `UserPublic.emailVerified`
  - [ ] Mobile (Expo Go): `expo-auth-session` Google button + OTP `verify.tsx` screen + onboarding `profile-setup.tsx`
  - [ ] ✅ Verify: register → code in Mailpit → verify → onboarding (name+DOB) → app; Google sign-in → tokens; on-device flows in Expo Go
...
- 2026-06-30 — Phase 0: **implemented Profile onboarding API (doc `15` §C) + the mobile OTP/onboarding flow.** Migration `20260630174317_profiles` on Neon (`profiles`, PK=FK to users, cascade). shared-types `profile.schema` (`profileSchema`/`ProfilePublic`/`Units`/`Sex`/`Goal`/`ActivityLevel`, `MIN_AGE_YEARS=13`, `isAtLeastAge`). New api `me` module: JWT-guarded `GET /me/profile` (404 if unset) + `POST /me/profile` (upsert; DOB stored `@db.Date`). Mobile: fixed signup flow for the no-token register (`register`→`RegisterResult`); `auth-context` adds `verifyEmail`/`resendCode`, `signUp` no longer authenticates; new `(auth)/verify.tsx` (OTP entry + resend cooldown) and `(onboarding)/profile-setup.tsx` (name+DOB); login routes `EMAIL_NOT_VERIFIED`→verify; `profile.tsx` has-profile gate→onboarding. API `tsc`+`nest build` clean; mobile code compile-clean except 2 **pre-existing** redesign-detour errors (`@react-navigation/native` missing, `text-field` TextInput overload). **Not runtime-tested, not committed.**
- 2026-06-30 — Phase 0: **implemented the API email-OTP layer (doc `15` §B).** Deps `nodemailer 9.0.3` + `@nestjs/throttler 6.5.0` + `@types/nodemailer 8.0.1` (exact). Migration `20260630172018_auth_otp` applied on Neon (`users.email_verified_at` nullable + `verification_codes` table + `(identifier,purpose)` index). Env: `SMTP_*` (optional → dev-log fallback) + `OTP_TTL`/`OTP_MAX_ATTEMPTS`/`OTP_RESEND_COOLDOWN`. New `notifications` module (`OtpDelivery` interface + SMTP/log/SMS-stub providers; selects SMTP when `SMTP_HOST` set, else logs code to console) + `otp.service` (sha-256 hashed codes, expiry/attempt-cap/cooldown, constant-time, non-enumerating). Auth wired: register issues NO tokens (sends code, returns `RegisterResult`); `verify-otp`→sets `emailVerifiedAt`+tokens; `resend-otp` (silent, throttled); unverified login→`403 EMAIL_NOT_VERIFIED`. Filter honors explicit `code`. Throttler on `/auth/*`. `tsc`+`nest build` clean. **Not runtime-tested, not committed.** Next: Profile onboarding (§C, 2nd migration) + mobile verify screen, then Google OAuth (§A).
- 2026-06-30 — Phase 0: **implemented core auth (doc `14`)**. API: migration `20260630100137_auth` (nullable `password_hash` + `refresh_tokens`); argon2id + JWT access + opaque rotating refresh (SHA-256 hashed, reuse-detection); register/login/refresh/logout/me; `ZodValidationPipe`; filter `details` passthrough; Swagger Bearer; `tsc`+`build` clean. Mobile: `expo-secure-store@~15.0.8`, `apiRequest`/`ApiError`, `AuthProvider`/`useAuth` (401→refresh→retry), `AuthForm`, login/register/profile screens. Deps pinned: `argon2 0.44.0`, `@nestjs/jwt 11.0.2`, `@nestjs/passport 11.0.5`, `passport 0.7.0`, `passport-jwt 4.0.1`. **OTP layer (doc `15`) started:** shared-types OTP schemas + `UserPublic.emailVerified`; delivery design locked (SMTP + dev-log sender). Onboarding/`Profile` decision recorded (name+DOB required, rest skippable). **Detour next:** mobile redesign (doc `16`). Not committed.
- 2026-06-30 — Planning: authored `15-auth-oauth-otp-plan.md` extending the Auth module with **Google OAuth + email OTP** on a **100%-open-source/free** path. Locked: Google via **ID-token** verification (mobile `expo-auth-session` PKCE in **Expo Go**, api `google-auth-library`); OTP **email-only now**, channel-agnostic (`nodemailer`→**Mailpit** dev, SMS provider stubbed/deferred — no free OSS SMS). Data model adds nullable `passwordHash`, `emailVerifiedAt`, `phone`/`phoneVerifiedAt`, `oauth_accounts`, `verification_codes`. Added Auth sub-tasks + 2 decisions above. No code/installs yet.
